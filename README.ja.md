...
仕様はv1.2。リポジトリには4本の単一ファイルPythonユーティリティ（標準ライブラリのみ、合計約1,000行）。
...
- [`orp_reader.py`](orp_reader.py) ── 単一ファイルreader（ライブラリ + CLI）
- [`orp_health.py`](orp_health.py) ── スキーマ + 鮮度バリデータ
- [`orp_link_check.py`](orp_link_check.py) ── wikilink 整合性スキャナ
- [`INSTALL.md`](INSTALL.md) ── インストール、4種類のトリガー、各エージェント接続方法
- [`OBSIDIAN-RAG-PROTOCOL.md`](OBSIDIAN-RAG-PROTOCOL.md) ── 完全なプロトコル仕様（v1.2）
