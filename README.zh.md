...
协议 v1.2。仓库里 4 个单文件 Python 工具，纯 stdlib，加起来约 1000 行。
...
- [`orp_reader.py`](orp_reader.py) ——单文件 reader（库 + CLI）
- [`orp_health.py`](orp_health.py) ——schema + 新鲜度校验
- [`orp_link_check.py`](orp_link_check.py) ——wikilink 完整性扫描
- [`INSTALL.md`](INSTALL.md) ——安装、4 种触发方式、各 agent 接入
- [`OBSIDIAN-RAG-PROTOCOL.md`](OBSIDIAN-RAG-PROTOCOL.md) ——完整协议 spec（v1.2）
